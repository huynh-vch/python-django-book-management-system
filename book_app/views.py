...
def edit_book_data(request,id):
    try:
        if request.method == "POST":
            std=Book.objects.get(book_title=request.session["id"]   ) 
            form = BookForm((request.POST),instance=std)
            if form.is_valid():
                form.save()
            del request.session['id']
            return redirect("/view_books")
        else:
            book_to_edit=BookInstance.objects.get(id=id)
            book=BookForm(instance=book_to_edit.book)
            # book = book_to_edit.book
            request.session["id"]=book_to_edit.book.book_title
            return render(request,'edit_books_data.html',{'book':book})
    except Exception as error:
        print(f"{error} occured at edit_books_data view")   

def delete_student(request,roll):
    std=Students.objects.get(roll_number=roll)
    std.delete()
    return redirect("/show_students")

def delete_book(request,id):
    std=BookInstance.objects.get(id=id)
    std.delete()
    return redirect("/view_books")

def return_issued_book(request,id):   
    obj=Book_Issue.objects.get(id=id)
    bis=BookInstance.objects.get(id=obj.book_instance.id)
    bis.Is_borrowed=False
    bis.save()
    return redirect('/view_books_issued')
